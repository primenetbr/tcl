...
rm_apps_rt51(){
	...
	if [ -e "rm_apps_rt51.list" ]; then
		...
		wget https://raw.githubusercontent.com/primenetbr/tcl/main/apps-list/rm_apps_rt51.list && clear
		if [ -e "rm_apps_rt51.list" ]; then
			for app_rm in $(cat rm_apps_rt51.list); do
	...
	if [ -e "rm_apps_rt51-uninstall.list" ]; then
		...
		# Baixar lista lixo dos apps
		echo -e " ${BLU}*${STD} ${NEG}Aguarde, baixando lista negra de apps...${STD}" && sleep 2
		wget https://raw.githubusercontent.com/primenetbr/tcl/main/apps-list/rm_apps_rt51-uninstall.list && clear
		if [ -e "rm_apps_rt51-uninstall.list" ]; then
			for app_rm in $(cat rm_apps_rt51-uninstall.list); do
				fakeroot adb shell pm uninstall --user 0 $app_rm >/dev/null
				if [ "$?" -eq "0" ]; then
...
enableApps() {
	...
	wget https://raw.githubusercontent.com/primenetbr/tcl/main/apps-list/apps_disable.list
...
install_CustomLauncher() {
    ...
    wget "https://github.com/primenetbr/tcl/raw/main/prebuilt/${1}.apk" -O "${1}.apk"
...
install_netmod() {
	...
	echo ""
	echo -e " ${BLU}*${STD} ${NEG}Baixando o ${1}...${STD}" && sleep 1
	wget https://github.com/primenetbr/tcl/raw/main/prebuilt/"${1}".apk && clear
	if [ "$?" -ne 0 ]; then
		echo ""
		echo -e " ${RED}*${STD} ${NEG}Erro ao baixar o arquivo. Verifique sua conexão ou tente mais tarde.${STD}"
	else
...
menu_InstallApps() { 
	...
	until [ "$option" = "10" ]; do
		...
		echo -e "  INSTALAR"
		echo -e "${ROX027}═════════════════════════════════════════════${STD}"
		echo -e " ${BLU}1.${STD} Youcine"
		echo -e " ${BLU}2.${STD} Spotify-Mod"
		echo -e " ${BLU}3.${STD} Downloader"
		echo -e " ${BLU}4.${STD} SmartTube"
		echo -e " ${BLU}5.${STD} SendFiles"
                echo -e " ${BLU}6.${STD} NetMod [VPN]"  
		echo -e " ${BLU}7.${STD} Launcher Setting"
                echo -e "${ROX027}═════════════════════════════════════════════${STD}"
		echo -e " ${BLU}0.${STD} ${ROX063}Voltar ao Menu Principal${STD}"
		echo -e "${ROX027}═════════════════════════════════════════════${STD}"
		read -p " Digite um Número: " option
		case $option in
			1 ) install_youcine "cinetv_homeoriginal-pm" ;;
			2 ) install_App "Spotify" ;;
